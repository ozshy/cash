...
## libraries
library(xtable)# data frames to LaTeX table
...
### Begin: currency in circulation (CIC) over GDP graph
#
# read gdp nominal domestic currency
#gdpn1 = read_excel("gdp_domestic_currency.xlsx", col_names = F)
#str(gdpn1)
#saveRDS(gdpn1, "gdp_domestic_currency.rds")
gdpn2.df = readRDS("gdp_domestic_currency.rds")
#
# read gdp real domestic currency
#gdpr1 = read_excel("gdp_real_domestic_currency.xlsx", col_names = F)
#str(gdpr1)
#saveRDS(gdpr1, "gdp_real_domestic_currency.rds")
gdpr2.df = readRDS("gdp_real_domestic_currency.rds")
#
# read CIC domestic currency (excluding dollar and euro areas, call it "o" for "other")
#cico1 = read_excel("cic_domestic_currency.xlsx", col_names = F)
#str(cico1)
#saveRDS(cico1, "cic_domestic_currency.rds")
cico2.df = readRDS("cic_domestic_currency.rds")
#
# read CIC euro area (call it "e" for "euro")
#cice1 = read_excel("cic_euro_area.xlsx", col_names = F)
#str(cice1)
#saveRDS(cice1, "cic_euro_area.rds")
cice2.df = readRDS("cic_euro_area.rds")
#
# read CIC dollar area (call it "d" for "euro")
#cicd1 = read_excel("cic_dollar_area.xlsx", col_names = F)
#str(cicd1)
#saveRDS(cicd1, "cic_dollar_area.rds")
cicd2.df = readRDS("cic_dollar_area.rds")

objects()

# delete 1st rows in the 3 cic dataframes
head(cico2.df,5)
cico3.df = as.data.frame(cico2.df[-1,])
#
head(cice2.df,5)
cice3.df = as.data.frame(cice2.df[-1,])
#
head(cicd2.df,5)
cicd3.df = as.data.frame(cicd2.df[-1,])
# 
# compare range of years
cico3.df[1,]# 2001-2020
cice3.df[1,]# 2000-2020
cicd3.df[1,]# 2001-2020
# remove year 2000 from cice
dim(cice3.df)
cice4.df = cice3.df[,-2]
cice4.df[1,]# 2001-2020
# 
dim(cico3.df)
dim(cice4.df)
dim(cicd3.df)
#
# to bind these files, I need to remove the "year" row from 2 cic files
cice5.df = cice4.df[-1, ]
head(cice5.df, 5)
head(cicd3.df, 5)
cicd4.df = cicd3.df[-1, ]
...
dim(cice6.df)
dim(cicd6.df)
# for row bind, all should have the same column names.
colnames(cice6.df) = colnames(cico6.df) #recall that I moved year 2000 from cice, so col num 2 was gone
# bind all 3 to have 144+4+5 regions
c1.df = rbind(cico6.df, cice6.df, cicd6.df)
head(c1.df,5)
dim(c1.df)
c1.df[,1] # names of counties/regions
c1.df[1,1]="Country"
str(c1.df)
# convert char numbers to numeric
c2.df = c1.df
c2.df[,2] = as.numeric(c1.df[,2])
c2.df[,3] = as.numeric(c1.df[,3])
c2.df[,4] = as.numeric(c1.df[,4])
c2.df[,5] = as.numeric(c1.df[,5])
c2.df[,6] = as.numeric(c1.df[,6])
c2.df[,7] = as.numeric(c1.df[,7])
c2.df[,8] = as.numeric(c1.df[,8])
c2.df[,9] = as.numeric(c1.df[,9])
c2.df[,10] = as.numeric(c1.df[,10])
c2.df[,11] = as.numeric(c1.df[,11])
c2.df[,12] = as.numeric(c1.df[,12])
c2.df[,13] = as.numeric(c1.df[,13])
c2.df[,14] = as.numeric(c1.df[,14])
c2.df[,15] = as.numeric(c1.df[,15])
c2.df[,16] = as.numeric(c1.df[,16])
c2.df[,17] = as.numeric(c1.df[,17])
c2.df[,18] = as.numeric(c1.df[,18])
c2.df[,19] = as.numeric(c1.df[,19])
c2.df[,20] = as.numeric(c1.df[,20])
c2.df[,21] = as.numeric(c1.df[,21])
#
# removing countries with missing year = 2001 obs
c3.df = c2.df
names(c3.df)
c3.df[1,]
colnames(c3.df) = c3.df[1,]# name col by year (also first row)
c4.df = c3.df[-1,]# del 1st row (now col names)
head(c4.df, 3)
c3.df[is.na(c3.df$`2001`),1]

# STOPPED HERE

c3.df[!complete.cases(c3.df),1]


which(is.na(c3.df), arr.ind = T)
...
c4.df = na.locf(c3.df, option = nobc)
head(c4.df)

#
c3.df[2,2:21] = na.interpolation(c2.df[2,2:21])
c2.df[2,]
c3.df[2,]


dim(c2.df)
c2.df[2:nrow, 2:21] = as.numeric(c1.df[2:nrow, 2:21] )


dim(c2.df)# only 145 countries
dim(gn2.df)# > 145
dim(gr2.df)# > 145

# working on c2 (CIC)
head(c2.df)
c3.df = c2.df[-1, ]# del 1st row
head(c3.df)
c3.df[1,1] = "country"
c4.df = as.data.frame(c3.df)
head(c4.df)
str(c4.df)
c4.df[,1]# list of countries
